...
- [x] Chạy song song 2 GPU (theo yêu cầu user, tăng tốc) — mỗi GPU 1 bản model riêng (`MODELS = {0: ..., 1: ...}`, không dùng chung 1 `device_map="auto"` vì cách đó không chạy song song được), chia đều đoạn cho 2 luồng qua `ThreadPoolExecutor`/`run_parallel()`. Tự động lùi về 1 GPU (hoặc CPU) nếu Kaggle không cấp đủ 2 GPU, không lỗi. Đã **mô phỏng chạy thử thật logic song song** (giả lập model, 20 đoạn giả gồm 2 lỗi cố ý) — xác nhận: đúng số kết quả, lỗi từng đoạn không làm sập cả batch, chia đều 2 GPU, không trùng/sót đoạn nào. Phần model/generate thật vẫn **chưa chạy được trên GPU thật** (như đã ghi ở trên).
- [ ] User tải notebook đã sửa lên Kaggle (thay bản cũ), bật **GPU T4 x2** (không phải T4 đơn/P100 — cần 2 GPU thật để tăng tốc), chạy lại smoke test xác nhận không còn câu hỏi kiểu dev-facing, rồi chạy toàn bộ, tải kết quả về đưa lại cho tôi
