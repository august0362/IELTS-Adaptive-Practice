...
- [x] Sửa lỗi phát hiện sau khi chốt milestone: `ThemeProvider.tsx` vẫn đặt `color-scheme: dark` cho theme tối (sót lại từ trước khi tách giao diện) — khiến trình duyệt tự vẽ `<select>`/date picker/spinner số theo giao diện tối gốc dù nền trang đã đúng là sáng. Sửa thành luôn `"light"` vì trang không còn chế độ tối thật sự nữa.
- [x] Sửa lỗi: xóa 1 lượt quay trong "Lượt quay gần đây" không reset khung cascade ở trên (vẫn hiện kỹ năng/part/dạng bài đã bị xóa như đang được chọn) — `Spinner.tsx` giờ có `resetCascadeDisplay()` dùng chung, gọi cả lúc bắt đầu quay mới lẫn sau khi xóa lượt quay (đưa `phase` về `idle`)
- [x] Thêm tính năng còn thiếu: trước đây chỉ nhập được số câu đúng ngay lúc vừa quay/tự học xong — nếu bỏ qua thì không còn cách nào quay lại nhập. `RecentRolls.tsx` giờ hiện nút "+ Nhập số câu đúng" cho mỗi kết quả Reading/Listening chưa có dữ liệu, mở `AccuracyEntry` ngay trong dòng lịch sử đó; `AccuracyEntry`'s `onSaved` giờ trả về số vừa lưu để hiện lại ngay không cần tải lại lịch sử
