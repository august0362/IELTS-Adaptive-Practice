...
5. `fetch` sang `ai/server/` (`POST {AI_SERVER_URL:-http://127.0.0.1:8787}/chat`) với `{ message, history, dbContext, mode }`, timeout 300s (đo thật trên máy dev: Flash ~30–46s/câu có RAG; Pro — think bật + top_k=6 — tới 244s, từng gặp `httpx.ReadTimeout` thật ở timeout cũ 120s nên đã nâng lên khớp `ai/server/ollama_client.py`'s `CHAT_TIMEOUT` — xem `ai/AI_TASKS.md`).
...
**Test:** `pytest` (`ai/server/tests/`, 19 case) cho phần logic thuần (chunking/retrieval/prompt) — không cần Ollama chạy. `ChatWindow.test.tsx` (component, mock fetch) cho UI. `chat.spec.ts` (e2e) cho luồng thật `web/` → route → `AI_SERVER_URL` → hiển thị — dùng **AI server giả lập** (`web/src/tests/e2e/mockAiServer.ts`, chạy như 1 `webServer` riêng trong `playwright.config.ts` cùng lúc với app) thay vì Ollama thật, vì độ trễ thật (~46s/câu) sẽ làm bộ e2e chậm và phụ thuộc máy chạy CI/máy khác có cài Ollama hay không — cố ý đánh đổi: e2e chứng minh đúng phần nối dây của `web/`, không chứng minh chất lượng câu trả lời của model (việc đó xác nhận thủ công, xem trên).

**Chưa làm:** Milestone 7 (fine-tune) và Milestone 8 (chấm Writing/Speaking) — xem `AI_CHATBOT_PLAN.md`.
