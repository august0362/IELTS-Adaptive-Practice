...
**Milestone 6 (Chatbot AI: hạ tầng + RAG) XONG.** App gốc (Milestone 1–5 mở rộng) đã dời vào [`web/`](./web/) để module hóa, chatbot AI nằm riêng trong [`ai/`](./ai/) — 2 module độc lập, đổi cái này không ảnh hưởng cái kia. Chatbot đã chạy được thật (không phải mock): Ollama + Qwen3.5-4B-Instruct + RAG trên tài liệu dự án, đã thử với câu hỏi tiếng Việt thật và trả lời đúng. Toàn bộ ngữ cảnh/quyết định (phạm vi, model chọn, cách sinh dữ liệu train, lý do pháp lý loại phương án dùng Claude sinh data, 3 phát hiện quan trọng lúc build — thinking mode làm chậm 65 lần, độ trễ thật ~46s/câu, lỗi chunk đoạn văn quá dài, chia mốc 6/7/8...) nằm ở **[`AI_CHATBOT_PLAN.md`](./AI_CHATBOT_PLAN.md) + [`ai/AI_TASKS.md`](./ai/AI_TASKS.md) — đọc trước khi tiếp tục bất cứ việc gì liên quan chatbot**, kể cả khi ngữ cảnh hội thoại đã mất.
